namespace LwfixTest.Physics.Jitter2.Behavior;
...
public class SleepTests
{
    ...
    public void StaticBodySleepingAndWake()
    {
        ...
        Assert.False(body.IsActive, "Static body should always be inactive.");
    ...
    public void KinematicTriggerWake()
    {
        ...
        Helper.AdvanceWorld(world, 3, (Real)(1.0 / 100.0), false);
        Assert.False(box.IsActive, "Box should be inactive after settling.");
        ...
        Assert.True(box.IsActive, "Box should wake up after being touched by moving static body.");

        // Optionally verify that the platform can still move even though it is static (infinite mass).
        Assert.True(platform.Position.Y > 0, "Platform should have moved due to integration.");
    ...
    public void StackedSleeping()
    {
        ...
        {
            Assert.False(body.IsActive, $"Body at {body.Position} should be inactive.");
        }
        ...
        Assert.True(bodies[1].IsActive, "Middle body should be reactivated.");
        Assert.True(bodies[2].IsActive, "Top body should be reactivated due to contact propagation.");

        // Advance world to allow them to fall asleep again
        Helper.AdvanceWorld(world, 2, (Real)(1.0 / 100.0), false);

        // Verify that both dynamic bodies went inactive again
        Assert.False(bodies[1].IsActive, "Middle body should be inactive again.");
        Assert.False(bodies[2].IsActive, "Top body should be inactive again.");
    ...
    public void StackedWakeOnRemoval()
    {
        ...
        Assert.True(bodies[2].IsActive, "Top body should wake up after its support is removed.");
    ...
    public void StaticBaseMovementWakesDynamic()
    {
        ...
        Helper.AdvanceWorld(world, 3, (Real)(1.0 / 100.0), false);
        Assert.False(bottom.IsActive, "Bottom (static) body should be inactive after settling.");
        Assert.False(top.IsActive, "Top body should be inactive after settling.");

        // Move the static body downward slightly
        bottom.Position -= new JVector(0, (Real)0.5, 0);

        // Process one step so the contact system updates
        Helper.AdvanceWorld(world, 1, (Real)(1.0 / 100.0), false);

        // The top body should now wake up because its contact was disturbed
        Assert.True(top.IsActive, "Top body should wake up after its static support moves.");
    ...
    public void ForceSleepIsland_DeactivatesWholeIslandImmediately()
    {
        ...
        Assert.Same(island, body2.Island);
        Assert.True(world.Islands.IsActive(island));
        ...
        Assert.True(callbackSawInactiveIsland);
        Assert.False(world.Islands.IsActive(island));
        Assert.False(world.RigidBodies.IsActive(body1));
        Assert.False(world.RigidBodies.IsActive(body2));
        Assert.False(body1.IsActive);
        Assert.False(body2.IsActive);
        Assert.Equal(JVector.Zero, body1.Velocity);
        Assert.Equal(JVector.Zero, body2.Velocity);
    }
    ...
    public void ForceSleepIsland_ZeroesKinematicVelocityImmediately()
    {
        ...
        Assert.False(body.IsActive);
        Assert.False(world.Islands.IsActive(body.Island));
    ...
    public void IslandDeactivatedEventIsRaisedAfterIslandSleeps()
    {
        ...
        Assert.Equal(1, deactivated);
        Assert.True(callbackSawBody);
        Assert.True(callbackSawInactiveBody);
        Assert.True(callbackSawInactivePartition);
    ...
    public void IslandActivatedEventIsRaisedAfterSleepingIslandWakes()
    {
        ...
        Assert.True(callbackSawBody);
        Assert.True(callbackSawActiveBody);
        Assert.True(callbackSawActivePartition);
