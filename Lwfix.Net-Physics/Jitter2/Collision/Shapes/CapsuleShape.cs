...
public class CapsuleShape : RigidBodyShape
{
    ...
    /// </exception>
    public CapsuleShape(Real radius = default, Real length = default)
    {
        if (Fixed32.ToRaw(radius) == 0) radius = (Real)0.5;
        if (Fixed32.ToRaw(length) == 0) length = (Real)1.0;
    ...
    public override void CalculateMassInertia(out JMatrix inertia, out JVector com, out Real mass)
    {
        ...
        Real massSphere = (Real)(4.0 / 3.0) * Fixed32.PI * radius * radius * radius;
        Real massCylinder = Fixed32.PI * radius * radius * length;
