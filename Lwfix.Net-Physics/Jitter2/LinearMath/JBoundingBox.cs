...
/// </summary>
[StructLayout(LayoutKind.Explicit, Size = 6 * Precision.RealSize)]
public struct JBoundingBox(JVector min, JVector max) : IEquatable<JBoundingBox>
{
    // The original (Real)1e-12 literal underflows to zero in Fixed32 (epsilon ~= 2.3e-10).
    // Fixed32 is a struct and therefore cannot be a const, so this is a static readonly
    // field initialized from Fixed32.Epsilon, preserving the original cut-off intent.
