...
public partial struct JQuaternion(Real x, Real y, Real z, Real w) : IEquatable<JQuaternion>
{
    ...
    [FieldOffset(3 * Precision.RealSize)] public Real W = w;
    ...
    public static void ToAxisAngle(JQuaternion quaternion, out JVector axis, out Real angle)
    {
        ...
        Real epsilonSingularity = (Real)1e-6;
        ...
        if (angle > Fixed32.PI)
        {
            angle = (Real)2.0 * Fixed32.PI - angle;
            axis = -axis;
        }
    }
    ...
    {
        return MathR.Sqrt(X * X + Y * Y + Z * Z + W * W);
    }
    ...
    public static JQuaternion Inverse(in JQuaternion value)
    {
        ...
        // The original (Real)1e-12 cut-off underflows to zero in Fixed32 (epsilon ~= 2.3e-10).
        if (lengthSq < Fixed32.Epsilon) return Identity;
