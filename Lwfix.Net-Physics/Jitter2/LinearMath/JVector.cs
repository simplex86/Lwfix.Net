...
/// </summary>
[StructLayout(LayoutKind.Explicit, Size = 3 * Precision.RealSize)]
public partial struct JVector(Real x, Real y, Real z) : IEquatable<JVector>
{
    ...
    [FieldOffset(0 * Precision.RealSize)] public Real X = x;
    [FieldOffset(1 * Precision.RealSize)] public Real Y = y;
    [FieldOffset(2 * Precision.RealSize)] public Real Z = z;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static JVector NormalizeSafe(in JVector value, Real epsilonSquared = default)
    {
        // The original default of 1e-16 underflows to zero in Fixed32 (epsilon ~= 2.3e-10).
        // Fall back to Fixed32.Epsilon so the cut-off remains meaningful.
        if (Fixed32.ToRaw(epsilonSquared) == 0) epsilonSquared = Fixed32.Epsilon;
