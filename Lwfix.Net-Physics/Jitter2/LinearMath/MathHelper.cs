...
{
    /// <summary>
    /// Gets the sign of <paramref name="value"/> purely from its sign bit.
    /// </summary>
    /// <param name="value">The number to test.</param>
    /// <returns>
    /// <c>+1</c> when the value is non-negative, <c>-1</c> when it is negative.
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static int SignBit(Real value)
    {
        // Fixed32 stores its bits in a signed long (two's complement). The sign
        // bit is bit 63. Shifting the raw value arithmetic-right by 63 yields
        // 0 for non-negative inputs and -1 for negative inputs; OR-ing with 1
        // collapses those to +1 / -1, matching the original IEEE-754 helper.
        return 1 | (int)(Fixed32.ToRaw(value) >> 63);
    }

    /// <summary>
    /// Calculates the rotation quaternion corresponding to the given angular velocity.
    /// </summary>
    ...
    public static JQuaternion RotationQuaternion(in JVector omega, Real dt)
    {
        ...
        // 1e-3 is comfortably above Fixed32 epsilon (~2.3e-10), so the literal survives.
        if (theta < (Real)1e-3)
        {
            Real dt3 = dt * dt * dt;
    ...
    public static bool IsRotationMatrix(in JMatrix matrix, Real epsilon = default)
    {
        if (Fixed32.ToRaw(epsilon) == 0) epsilon = (Real)1e-06;
    ...
    public static bool IsZero(in JVector vector, Real epsilon = default)
    {
        if (Fixed32.ToRaw(epsilon) == 0) epsilon = (Real)1e-6;
    ...
    public static bool IsZero(Real value, Real epsilon = default)
    {
        if (Fixed32.ToRaw(epsilon) == 0) epsilon = (Real)1e-6;
    ...
    public static bool UnsafeIsZero(ref JMatrix matrix, Real epsilon = default)
    {
        if (Fixed32.ToRaw(epsilon) == 0) epsilon = (Real)1e-6;
    ...
    public static JVector CreateOrthonormal(in JVector vec)
    {
        ...
        // Math.Abs here is operating on Real (Fixed32). Fixed32 exposes its own
        // Abs, so route through MathR (which is FMath) to keep this generic-friendly.
    ...
    public static bool CheckOrthonormalBasis(in JMatrix matrix, Real epsilon = default)
    {
        if (Fixed32.ToRaw(epsilon) == 0) epsilon = (Real)1e-6;
        JMatrix delta = JMatrix.MultiplyTransposed(matrix, matrix) - JMatrix.Identity;
    ...
    public static bool CloseToZero(in JVector v, Real epsilonSq = default)
    {
        // The original default of 1e-16 underflows to zero in Fixed32 (epsilon ~= 2.3e-10).
        // Falling back to Fixed32.Epsilon preserves the "anything strictly above zero
        // qualifies" intent without producing a dead comparison.
        if (Fixed32.ToRaw(epsilonSq) == 0) epsilonSq = Fixed32.Epsilon;
