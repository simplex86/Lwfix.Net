...
public unsafe struct CollisionManifold
{
    ...
    private static readonly Real DuplicatePointDistanceSq = (Real)0.001;

    // Layout: MaxManifoldPoints on shape A followed by the matching points on shape B.
    // CS1663: Fixed32 is a struct and cannot be used as a fixed-size buffer element type.
    // Use `fixed long` (same 8-byte size) and reinterpret via Unsafe.As when accessing.
    private fixed long manifoldData[MaxManifoldPoints * 2 * 3];
    ...
    private static readonly Real Sqrt3Over2 = (Real)0.86602540378;
    // Small perturbation used to sample support points around the contact normal.
    private static readonly Real Perturbation = (Real)0.01;
    ...
    /// </summary>
    public Span<JVector> ManifoldA => MemoryMarshal.CreateSpan(ref Unsafe.As<long, JVector>(ref manifoldData[0]), manifoldCount);

    /// <summary>
    /// Gets the contact points on shape B.
    /// </summary>
    public Span<JVector> ManifoldB => MemoryMarshal.CreateSpan(ref Unsafe.As<long, JVector>(ref manifoldData[MaxManifoldPoints * 3]), manifoldCount);
        ...
        ReadOnlySpan<JVector> polygon, int polygonCount, in JVector normal, Real sign, Span<JVector> clipped)
    {
        ...
        for (int i = 0; i < polygonCount; i++)
        {
            ...
            Real t = MathR.Clamp(startSide / denominator, (Real)0.0, (Real)1.0);
        ...
        Span<JVector> clippedA, Span<JVector> clippedB)
    {
        ...
        if (MathR.Abs(cross) <= epsilon)
        {
            ...
            Real parameterA0 = MathR.Clamp(JVector.Dot(clippedA[0] - startB, deltaB) / lengthBSquared, (Real)0.0, (Real)1.0);
            ...
            Real parameterA1 = MathR.Clamp(JVector.Dot(clippedA[1] - startB, deltaB) / lengthBSquared, (Real)0.0, (Real)1.0);
        ...
        t = MathR.Clamp(t, (Real)0.0, (Real)1.0);
        u = MathR.Clamp(u, (Real)0.0, (Real)1.0);
    ...
    private static void ReduceManifold(Span<JVector> manifoldA, Span<JVector> manifoldB, ref int manifoldCount, in JVector normal)
    {
        ...
        Span<Real> angles = stackalloc Real[MaxManifoldPoints];
        ...
        {
            int current = order[i];
            Real currentAngle = angles[current];
            int j = i - 1;
        ...
        where Ta : ISupportMappable where Tb : ISupportMappable
    {
        ...
        Span<JVector> mA = MemoryMarshal.CreateSpan(ref Unsafe.As<long, JVector>(ref manifoldData[0]), MaxManifoldPoints);
        Span<JVector> mB = MemoryMarshal.CreateSpan(ref Unsafe.As<long, JVector>(ref manifoldData[MaxManifoldPoints * 3]), MaxManifoldPoints);
