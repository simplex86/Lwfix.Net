...
{
    private static readonly Real GoldenRatio = (Real)1.6180339887498948482045;
        ...
        int subdivisions) where TSupport : ISupportMappable where TSink : ISink<JTriangle>
    {
        if (subdivisions <= 1)
        {
            ...
            if (n.LengthSquared() > Fixed32.Epsilon)
            {
    ...
    /// <inheritdoc cref="Tessellate(System.ReadOnlySpan{SimplexLab.LwfixPhysics.Jitter2.LinearMath.JVector}, int)"/>
        ...
        where TSupport : ISupportMappable
    {
        ...
        Real a = (Real)(1.0 / 60.0), b = (Real)(1.0 / 120.0);
