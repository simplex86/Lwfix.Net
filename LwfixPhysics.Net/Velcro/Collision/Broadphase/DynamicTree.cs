    ...
    internal class DynamicTree<T>
    {
        ...
        public Fixed32 AreaRatio
        {
            get
            {
                ...
                Fixed32 rootArea = root.AABB.Perimeter;

                Fixed32 totalArea = (Fixed32)0.0;
        ...
        public bool MoveProxy(int proxyId, ref AABB aabb, Vector2 displacement)
        {
            ...
            if (treeAABB.Contains(ref aabb))
            {
                ...
                hugeAABB.LowerBound = fatAABB.LowerBound - (Fixed32)4.0 * r;
                hugeAABB.UpperBound = fatAABB.UpperBound + (Fixed32)4.0 * r;
        ...
        public void RayCast(Func<RayCastInput, int, Fixed32> callback, ref RayCastInput input)
        {
            Vector2 p1 = input.Point1;
            Vector2 p2 = input.Point2;
            Vector2 r = p2 - p1;
            Debug.Assert(r.LengthSquared() > (Fixed32)0.0);
            r.Normalize();
            ...
            Fixed32 maxFraction = input.MaxFraction;
            ...
            while (_raycastStack.Count > 0)
            {
                ...
                Fixed32 separation = FMath.Abs(Vector2.Dot(new Vector2(-r.Y, r.X), p1 - c)) - Vector2.Dot(absV, h);
                if (separation > (Fixed32)0.0)
                    continue;
                ...
                if (node.IsLeaf())
                {
                    ...
                    Fixed32 value = callback(subInput, nodeId);

                    if (value == (Fixed32)0.0)
                    {
        ...
        private void InsertLeaf(int leaf)
        {
            ...
            while (!_nodes[index].IsLeaf())
            {
                ...
                Fixed32 area = _nodes[index].AABB.Perimeter;

                AABB combinedAABB = new AABB();
                combinedAABB.Combine(ref _nodes[index].AABB, ref leafAABB);
                Fixed32 combinedArea = combinedAABB.Perimeter;

                // Cost of creating a new parent for this node and the new leaf
                Fixed32 cost = (Fixed32)2.0 * combinedArea;

                // Minimum cost of pushing the leaf further down the tree
                Fixed32 inheritanceCost = (Fixed32)2.0 * (combinedArea - area);
                ...
                else
                {
                    AABB aabb = new AABB();
                    aabb.Combine(ref leafAABB, ref _nodes[child1].AABB);
                    Fixed32 oldArea = _nodes[child1].AABB.Perimeter;
                    Fixed32 newArea = aabb.Perimeter;
                ...
                else
                {
                    AABB aabb = new AABB();
                    aabb.Combine(ref leafAABB, ref _nodes[child2].AABB);
                    Fixed32 oldArea = _nodes[child2].AABB.Perimeter;
                    Fixed32 newArea = aabb.Perimeter;
        ...
        public void RebuildBottomUp()
        {
            ...
            while (count > 1)
            {
                Fixed32 minCost = MathConstants.MaxFloat;
                int iMin = -1, jMin = -1;
