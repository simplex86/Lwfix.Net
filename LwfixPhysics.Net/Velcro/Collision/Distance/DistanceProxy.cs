...
namespace SimplexLab.LwfixPhysics.Velcro.Collision.Distance
{
    ...
    public struct DistanceProxy
    {
        ...
        public DistanceProxy(Vector2[] vertices, Fixed32 radius)
        {
            _vertices = vertices;
            _radius = radius;
        }
        ...
        {
            int bestIndex = 0;
            Fixed32 bestValue = Vector2.Dot(_vertices[0], direction);
            for (int i = 1; i < _vertices.Length; ++i)
            {
                Fixed32 value = Vector2.Dot(_vertices[i], direction);
                if (value > bestValue)
                {
                    bestIndex = i;
                    bestValue = value;
