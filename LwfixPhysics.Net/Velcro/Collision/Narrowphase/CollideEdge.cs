...
namespace SimplexLab.LwfixPhysics.Velcro.Collision.Narrowphase
{
    internal static class CollideEdge
    {
        ...
        public static void CollideEdgeAndCircle(ref Manifold manifold, EdgeShape edgeA, ref Transform transformA, CircleShape circleB, ref Transform transformB)
        {
            ...
            Fixed32 offset = MathUtils.Dot(n, Q - A);

            bool oneSided = edgeA.OneSided;
            if (oneSided && offset < (Fixed32)0.0)
                return;

            // Barycentric coordinates
            Fixed32 u = Vector2.Dot(e, B - Q);
            Fixed32 v = Vector2.Dot(e, Q - A);

            Fixed32 radius = edgeA._radius + circleB._radius;
            ...
            if (v <= (Fixed32)0.0)
            {
                ...
                if (edgeA.OneSided)
                {
                    ...
                    Vector2 B1 = A;
                    Vector2 e1 = B1 - A1;
                    Fixed32 u1 = Vector2.Dot(e1, B1 - Q);
            ...
            if (u <= (Fixed32)0.0)
            {
                ...
                if (edgeA.OneSided)
                {
                    ...
                    Vector2 A2 = B;
                    Vector2 e2 = B2 - A2;
                    Fixed32 v2 = Vector2.Dot(e2, Q - A2);
            ...
            // Region AB
            Fixed32 den = Vector2.Dot(e, e);
            Debug.Assert(den > (Fixed32)0.0);
            Vector2 P = ((Fixed32)1.0 / den) * (u * A + v * B);
            Vector2 d = Q - P;
            Fixed32 dd = Vector2.Dot(d, d);
            if (dd > radius * radius)
                return;

            if (offset < (Fixed32)0.0)
        ...
        public static void CollideEdgeAndPolygon(ref Manifold manifold, EdgeShape edgeA, ref Transform xfA, PolygonShape polygonB, ref Transform xfB)
        {
            ...
            Fixed32 offset1 = MathUtils.Dot(normal1, centroidB - v1);

            bool oneSided = edgeA._oneSided;
            if (oneSided && offset1 < (Fixed32)0.0)
            {
                return;
            }
            ...
            Fixed32 radius = polygonB._radius + edgeA._radius;
            ...
            Fixed32 k_relativeTol = (Fixed32)0.98;
            Fixed32 k_absoluteTol = (Fixed32)0.001;
            ...
            if (oneSided)
            {
                ...
                bool convex1 = MathUtils.Cross(edge0, edge1) >= (Fixed32)0.0;

                Vector2 edge2 = edgeA._vertex3 - v2;
                edge2.Normalize();
                Vector2 normal2 = new Vector2(edge2.Y, -edge2.X);
                bool convex2 = MathUtils.Cross(edge1, edge2) >= (Fixed32)0.0;

                Fixed32 sinTol = (Fixed32)0.1;
                bool side1 = MathUtils.Dot(primaryAxis.Normal, edge1) <= (Fixed32)0.0;
            ...
            if (primaryAxis.Type == EPAxisType.EdgeA)
            {
                ...
                int bestIndex = 0;
                Fixed32 bestValue = MathUtils.Dot(primaryAxis.Normal, tempPolygonB.Normals[0]);
                for (int i = 1; i < tempPolygonB.Count; ++i)
                {
                    Fixed32 value = MathUtils.Dot(primaryAxis.Normal, tempPolygonB.Normals[i]);
                    if (value < bestValue)
                    {
                        bestValue = value;
                        bestIndex = i;
            ...
            {
                Fixed32 separation = MathUtils.Dot(ref1.Normal, clipPoints2[i].V - ref1.v1);
        ...
        private static EPAxis ComputeEdgeSeparation(ref TempPolygon polygonB, Vector2 v1, Vector2 normal1)
        {
            ...
            for (int j = 0; j < 2; ++j)
            {
                Fixed32 sj = MathConstants.MaxFloat;

                // Find deepest polygon vertex along axis j
                for (int i = 0; i < polygonB.Count; ++i)
                {
                    Fixed32 si = MathUtils.Dot(axes[j], polygonB.Vertices[i] - v1);
                    if (si < sj)
                    {
                        sj = si;
                    }
        ...
        private static EPAxis ComputePolygonSeparation(ref TempPolygon polygonB, Vector2 v1, Vector2 v2)
        {
            ...
            for (int i = 0; i < polygonB.Count; ++i)
            {
                ...
                Fixed32 s1 = MathUtils.Dot(n, polygonB.Vertices[i] - v1);
                Fixed32 s2 = MathUtils.Dot(n, polygonB.Vertices[i] - v2);
                Fixed32 s = MathUtils.Min(s1, s2);
