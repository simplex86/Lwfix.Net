...
namespace SimplexLab.LwfixPhysics.Velcro.Collision.Narrowphase
{
    public static class CollidePolygon
    {
        ...
        public static void CollidePolygons(ref Manifold manifold, PolygonShape polyA, ref Transform xfA, PolygonShape polyB, ref Transform xfB)
        {
            ...
            Fixed32 totalRadius = polyA._radius + polyB._radius;

            Fixed32 separationA = FindMaxSeparation(out int edgeA, polyA, ref xfA, polyB, ref xfB);
            if (separationA > totalRadius)
                return;

            Fixed32 separationB = FindMaxSeparation(out int edgeB, polyB, ref xfB, polyA, ref xfA);
            ...
            bool flip;
            Fixed32 k_tol = (Fixed32)0.1 * Settings.LinearSlop;
            ...
            Vector2 localNormal = MathUtils.Cross(localTangent, (Fixed32)1.0);
            Vector2 planePoint = (Fixed32)0.5 * (v11 + v12);

            Vector2 tangent = MathUtils.Mul(ref xf1.q, localTangent);
            Vector2 normal = MathUtils.Cross(tangent, (Fixed32)1.0);

            v11 = MathUtils.Mul(ref xf1, v11);
            v12 = MathUtils.Mul(ref xf1, v12);

            // Face offset.
            Fixed32 frontOffset = Vector2.Dot(normal, v11);

            // Side offsets, extended by polytope skin thickness.
            Fixed32 sideOffset1 = -Vector2.Dot(tangent, v11) + totalRadius;
            Fixed32 sideOffset2 = Vector2.Dot(tangent, v12) + totalRadius;
            ...
            {
                Fixed32 separation = Vector2.Dot(normal, clipPoints2[i].V) - frontOffset;
        ...
        private static Fixed32 FindMaxSeparation(out int edgeIndex, PolygonShape poly1, ref Transform xf1, PolygonShape poly2, ref Transform xf2)
        {
            int count1 = poly1._vertices.Count;
            ...
            int bestIndex = 0;
            Fixed32 maxSeparation = -MathConstants.MaxFloat;
            for (int i = 0; i < count1; ++i)
            {
                ...
                Fixed32 si = MathConstants.MaxFloat;
                for (int j = 0; j < count2; ++j)
                {
                    Fixed32 sij = Vector2.Dot(n, v2s[j] - v1);
                    if (sij < si)
                        si = sij;
                }
        ...
        private static void FindIncidentEdge(out FixedArray2<ClipVertex> c, PolygonShape poly1, ref Transform xf1, int edge1, PolygonShape poly2, ref Transform xf2)
        {
            ...
            int index = 0;
            Fixed32 minDot = MathConstants.MaxFloat;
            for (int i = 0; i < count2; ++i)
            {
                Fixed32 dot = Vector2.Dot(normal1, normals2[i]);
                if (dot < minDot)
                {
                    minDot = dot;
                    index = i;
