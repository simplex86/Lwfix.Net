...
namespace SimplexLab.LwfixPhysics.Velcro.Collision.Narrowphase
{
    ...
    internal static class Collision
    {
        ...
        public static bool TestOverlap(Shape shapeA, int indexA, Shape shapeB, int indexB, ref Transform xfA, ref Transform xfB)
        {
            ...
            return output.Distance < (Fixed32)10.0 * MathConstants.Epsilon;
        }
        ...
        /// <returns></returns>
        internal static int ClipSegmentToLine(out FixedArray2<ClipVertex> vOut, ref FixedArray2<ClipVertex> vIn, Vector2 normal, Fixed32 offset, int vertexIndexA)
        {
            ...
            Fixed32 distance0 = Vector2.Dot(normal, vIn.Value0.V) - offset;
            Fixed32 distance1 = Vector2.Dot(normal, vIn.Value1.V) - offset;

            // If the points are behind the plane
            if (distance0 <= (Fixed32)0.0) vOut[count++] = vIn.Value0;
            if (distance1 <= (Fixed32)0.0) vOut[count++] = vIn.Value1;

            // If the points are on different sides of the plane
            if (distance0 * distance1 < (Fixed32)0.0)
            {
                // Find intersection point of edge and plane
                Fixed32 interp = distance0 / (distance0 - distance1);
