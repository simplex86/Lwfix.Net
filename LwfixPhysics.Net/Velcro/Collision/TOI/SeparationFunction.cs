    ...
    public static class SeparationFunction
    {
        public static void Initialize(ref SimplexCache cache, DistanceProxy proxyA, ref Sweep sweepA, DistanceProxy proxyB, ref Sweep sweepB, Fixed32 t1, out Vector2 axis, out Vector2 localPoint, out SeparationFunctionType type)
        {
            ...
            else if (cache.IndexA[0] == cache.IndexA[1])
            {
                ...
                localPoint = (Fixed32)0.5 * (localPointB1 + localPointB2);
                Vector2 pointB = MathUtils.Mul(ref xfB, localPoint);

                Vector2 localPointA = proxyA._vertices[cache.IndexA[0]];
                Vector2 pointA = MathUtils.Mul(ref xfA, localPointA);

                Fixed32 s = Vector2.Dot(pointA - pointB, normal);
                if (s < (Fixed32)0.0)
                    axis = -axis;
            }
            else
            {
                ...
                localPoint = (Fixed32)0.5 * (localPointA1 + localPointA2);
                Vector2 pointA = MathUtils.Mul(ref xfA, localPoint);

                Vector2 localPointB = proxyB._vertices[cache.IndexB[0]];
                Vector2 pointB = MathUtils.Mul(ref xfB, localPointB);

                Fixed32 s = Vector2.Dot(pointB - pointA, normal);
                if (s < (Fixed32)0.0)
                    axis = -axis;
            }
        ...
        public static Fixed32 FindMinSeparation(out int indexA, out int indexB, Fixed32 t, DistanceProxy proxyA, ref Sweep sweepA, DistanceProxy proxyB, ref Sweep sweepB, ref Vector2 axis, ref Vector2 localPoint, SeparationFunctionType type)
        {
            ...
            switch (type)
            {
                case SeparationFunctionType.Points:
                {
                    ...
                    Fixed32 separation = Vector2.Dot(pointB - pointA, axis);
                    return separation;
                }
                ...
                case SeparationFunctionType.FaceA:
                {
                    ...
                    Fixed32 separation = Vector2.Dot(pointB - pointA, normal);
                    return separation;
                }
                ...
                case SeparationFunctionType.FaceB:
                {
                    ...
                    Fixed32 separation = Vector2.Dot(pointA - pointB, normal);
                    return separation;
                }
        ...
        public static Fixed32 Evaluate(int indexA, int indexB, Fixed32 t, DistanceProxy proxyA, ref Sweep sweepA, DistanceProxy proxyB, ref Sweep sweepB, ref Vector2 axis, ref Vector2 localPoint, SeparationFunctionType type)
        {
            sweepA.GetTransform(out Transform xfA, t);
            sweepB.GetTransform(out Transform xfB, t);
