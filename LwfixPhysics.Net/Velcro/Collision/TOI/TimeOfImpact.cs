...
namespace SimplexLab.LwfixPhysics.Velcro.Collision.TOI
{
    internal static class TimeOfImpact
    {
        ...
        public static void CalculateTimeOfImpact(ref TOIInput input, out TOIOutput output)
        {
            ...
            Fixed32 tMax = input.TMax;

            Fixed32 totalRadius = input.ProxyA._radius + input.ProxyB._radius;
            Fixed32 target = Fixed32.Max(Settings.LinearSlop, totalRadius - (Fixed32)3.0 * Settings.LinearSlop);
            Fixed32 tolerance = (Fixed32)0.25 * Settings.LinearSlop;
            Debug.Assert(target > tolerance);

            Fixed32 t1 = (Fixed32)0.0;
            ...
            for (;;)
            {
                ...
                if (distanceOutput.Distance <= (Fixed32)0.0)
                {
                    // Failure!
                    output.State = TOIOutputState.Overlapped;
                    output.T = (Fixed32)0.0;
                    break;
                }
                ...
                for (;;)
                {
                    ...
                    Fixed32 s2 = SeparationFunction.FindMinSeparation(out int indexA, out int indexB, t2, input.ProxyA, ref sweepA, input.ProxyB, ref sweepB, ref axis, ref localPoint, type);
                    ...
                    Fixed32 s1 = SeparationFunction.Evaluate(indexA, indexB, t1, input.ProxyA, ref sweepA, input.ProxyB, ref sweepB, ref axis, ref localPoint, type);
                    ...
                    for (;;)
                    {
                        ...
                        Fixed32 s = SeparationFunction.Evaluate(indexA, indexB, t, input.ProxyA, ref sweepA, input.ProxyB, ref sweepB, ref axis, ref localPoint, type);

                        if (FMath.Abs(s - target) < tolerance)
                        {
