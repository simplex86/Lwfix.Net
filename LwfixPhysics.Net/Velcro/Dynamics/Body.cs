...
namespace SimplexLab.LwfixPhysics.Velcro.Dynamics
{
    public class Body
    {
        ...
        public Fixed32 Revolutions => Rotation / MathConstants.Pi;
        ...
        public Vector2 LinearVelocity
        {
            ...
            set
            {
                Debug.Assert(!Fixed32.IsNaN(value.X) && !Fixed32.IsNaN(value.Y));

                if (_type == BodyType.Static)
                    return;

                if (Vector2.Dot(value, value) > Fixed32.Zero)
                    Awake = true;
        ...
        public Fixed32 AngularVelocity
        {
            get => _angularVelocity;
            set
            {
                Debug.Assert(!Fixed32.IsNaN(value));

                if (_type == BodyType.Static)
                    return;

                if (value * value > Fixed32.Zero)
                    Awake = true;
        ...
        public Fixed32 Inertia
        {
            ...
            set
            {
                ...
                if (value > Fixed32.Zero && !FixedRotation)
                {
                    _inertia = value - _mass * Vector2.Dot(_sweep.LocalCenter, _sweep.LocalCenter);
                    Debug.Assert(_inertia > Fixed32.Zero);
                    _invI = (Fixed32)1.0 / _inertia;
                }
            }
        }
        ...
        public Fixture AddFixture(FixtureDef def)
        {
            ...
            if (fixture._shape._density > Fixed32.Zero)
            {
                ResetMassData();
            }
        ...
        public void SetTransform(Vector2 position, Fixed32 rotation)
        {
        ...
        /// <param name="position">The world position of the body's local origin.</param>
        /// <param name="rotation">The world rotation in radians.</param>
        public void SetTransform(ref Vector2 position, Fixed32 rotation)
        {
            Debug.Assert(!_world._isLocked);
            if (_world._isLocked)
                return;
        ...
        {
            Debug.Assert(!Fixed32.IsNaN(force.X));
            Debug.Assert(!Fixed32.IsNaN(force.Y));
            Debug.Assert(!Fixed32.IsNaN(point.X));
            Debug.Assert(!Fixed32.IsNaN(point.Y));
        ...
        public void ApplyTorque(Fixed32 torque)
        {
            Debug.Assert(!Fixed32.IsNaN(torque));
        ...
        public void ApplyAngularImpulse(Fixed32 impulse)
        {
            if (_type != BodyType.Dynamic)
                return;
        ...
        public void ResetMassData()
        {
            ...
            {
                if (f.Shape._density == Fixed32.Zero)
                    continue;
            ...
            if (_inertia > Fixed32.Zero && (_flags & BodyFlags.FixedRotationFlag) == 0)
            {
        ...
        internal void Advance(Fixed32 alpha)
        {
