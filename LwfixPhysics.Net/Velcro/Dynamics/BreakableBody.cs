...
namespace SimplexLab.LwfixPhysics.Velcro.Dynamics
{
    ...
    public class BreakableBody
    {
        ...
        private Fixed32[] _angularVelocitiesCache = new Fixed32[8];
        private bool _break;
        private Vector2[] _velocitiesCache = new Vector2[8];

        public BreakableBody(World world, ICollection<Vertices> parts, Fixed32 density, Vector2 position = new Vector2(), Fixed32 rotation = default)
        {
            _world = world;
            _world.ContactManager.PostSolve += PostSolve;
            Parts = new List<Fixture>(parts.Count);
            MainBody = BodyFactory.CreateBody(_world, position, rotation, BodyType.Dynamic);
            Strength = (Fixed32)500.0;
        ...
        public BreakableBody(World world, IEnumerable<Shape> shapes, Vector2 position = new Vector2(), Fixed32 rotation = default)
        {
            _world = world;
            _world.ContactManager.PostSolve += PostSolve;
            MainBody = BodyFactory.CreateBody(_world, position, rotation, BodyType.Dynamic);
        ...
        private void PostSolve(Contact contact, ContactVelocityConstraint impulse)
        {
            if (!Broken)
            {
                ...
                {
                    Fixed32 maxImpulse = Fixed32.Zero;
                    int count = contact.Manifold.PointCount;

                    for (int i = 0; i < count; ++i)
                    {
                        maxImpulse = Fixed32.Max(maxImpulse, impulse.Points[i].NormalImpulse);
                    }
        ...
        public void Update()
        {
            ...
            if (!Broken)
            {
                ...
                if (Parts.Count > _angularVelocitiesCache.Length)
                {
                    ...
                    _angularVelocitiesCache = new Fixed32[Parts.Count];
                }
