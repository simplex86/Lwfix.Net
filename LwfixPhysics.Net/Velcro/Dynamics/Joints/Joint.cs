...
namespace SimplexLab.LwfixPhysics.Velcro.Dynamics.Joints
{
    public abstract class Joint
    {
        ...
        internal void Validate(Fixed32 invDt)
        {
            if (!_enabled)
                return;

            Fixed32 jointErrorSquared = GetReactionForce(invDt).LengthSquared();

            if (FMath.Abs(jointErrorSquared) <= _breakpoint * _breakpoint)
                return;

            _enabled = false;

            Broke?.Invoke(this, Fixed32.Sqrt(jointErrorSquared));
        }
