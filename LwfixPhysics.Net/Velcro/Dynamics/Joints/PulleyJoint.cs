...
namespace SimplexLab.LwfixPhysics.Velcro.Dynamics.Joints
{
    ...
    public class PulleyJoint : Joint
    {
        ...
        private Fixed32 _lengthA;
        private Fixed32 _lengthB;

        // Solver shared
        private Vector2 _localAnchorA;
        private Vector2 _localAnchorB;
        private Fixed32 _constant;
        private Fixed32 _ratio;
            ...
            : base(def)
        {
            ...
            Debug.Assert(def.Ratio != Fixed32.Zero);
            _ratio = def.Ratio;
        ...
        public PulleyJoint(Body bodyA, Body bodyB, Vector2 anchorA, Vector2 anchorB, Vector2 worldAnchorA, Vector2 worldAnchorB, Fixed32 ratio, bool useWorldCoordinates = false)
            : base(bodyA, bodyB, JointType.Pulley)
        {
            ...
            Debug.Assert(ratio != Fixed32.Zero);
        ...
        internal override void InitVelocityConstraints(ref SolverData data)
        {
            ...
            Fixed32 lengthA = _uA.Length();
            Fixed32 lengthB = _uB.Length();

            if (lengthA > (Fixed32)10.0 * Settings.LinearSlop)
                _uA *= (Fixed32)1.0 / lengthA;
            else
                _uA = Vector2.Zero;

            if (lengthB > (Fixed32)10.0 * Settings.LinearSlop)
                _uB *= (Fixed32)1.0 / lengthB;
            else
                _uB = Vector2.Zero;

            // Compute effective mass.
            Fixed32 ruA = MathUtils.Cross(_rA, _uA);
            Fixed32 ruB = MathUtils.Cross(_rB, _uB);

            Fixed32 mA = _invMassA + _invIA * ruA * ruA;
            Fixed32 mB = _invMassB + _invIB * ruB * ruB;
        ...
        internal override void SolveVelocityConstraints(ref SolverData data)
        {
            ...
            Fixed32 Cdot = -Vector2.Dot(_uA, vpA) - _ratio * Vector2.Dot(_uB, vpB);
        ...
        internal override bool SolvePositionConstraints(ref SolverData data)
        {
            ...
            Fixed32 lengthA = uA.Length();
            Fixed32 lengthB = uB.Length();

            if (lengthA > (Fixed32)10.0 * Settings.LinearSlop)
                uA *= (Fixed32)1.0 / lengthA;
            else
                uA = Vector2.Zero;

            if (lengthB > (Fixed32)10.0 * Settings.LinearSlop)
                uB *= (Fixed32)1.0 / lengthB;
            else
                uB = Vector2.Zero;

            // Compute effective mass.
            Fixed32 ruA = MathUtils.Cross(rA, uA);
            Fixed32 ruB = MathUtils.Cross(rB, uB);

            Fixed32 mA = _invMassA + _invIA * ruA * ruA;
            Fixed32 mB = _invMassB + _invIB * ruB * ruB;

            Fixed32 mass = mA + _ratio * _ratio * mB;

            if (mass > Fixed32.Zero)
                mass = (Fixed32)1.0 / mass;

            Fixed32 C = _constant - lengthA - _ratio * lengthB;
            Fixed32 linearError = FMath.Abs(C);

            Fixed32 impulse = -mass * C;
