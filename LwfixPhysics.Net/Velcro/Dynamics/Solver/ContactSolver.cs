    ...
    internal class ContactSolver
    {
        ...
        public void Reset(TimeStep step, int count, Contact[] contacts, Position[] positions, Velocity[] velocities)
        {
            ...
            for (int i = 0; i < _count; ++i)
            {
                ...
                Fixed32 radiusA = shapeA._radius;
                Fixed32 radiusB = shapeB._radius;
                ...
                for (int j = 0; j < pointCount; ++j)
                {
                    ...
                    {
                        vcp.NormalImpulse = Fixed32.Zero;
                        vcp.TangentImpulse = Fixed32.Zero;
                    }

                    vcp.rA = Vector2.Zero;
                    vcp.rB = Vector2.Zero;
                    vcp.NormalMass = Fixed32.Zero;
                    vcp.TangentMass = Fixed32.Zero;
                    vcp.VelocityBias = Fixed32.Zero;
        ...
        public void InitializeVelocityConstraints()
        {
            for (int i = 0; i < _count; ++i)
            {
                ...
                Fixed32 radiusA = pc.RadiusA;
                Fixed32 radiusB = pc.RadiusB;
                Manifold manifold = _contacts[vc.ContactIndex].Manifold;

                int indexA = vc.IndexA;
                int indexB = vc.IndexB;

                Fixed32 mA = vc.InvMassA;
                Fixed32 mB = vc.InvMassB;
                Fixed32 iA = vc.InvIA;
                Fixed32 iB = vc.InvIB;
                ...
                Fixed32 aA = _positions[indexA].A;
                Vector2 vA = _velocities[indexA].V;
                Fixed32 wA = _velocities[indexA].W;

                Vector2 cB = _positions[indexB].C;
                Fixed32 aB = _positions[indexB].A;
                Vector2 vB = _velocities[indexB].V;
                Fixed32 wB = _velocities[indexB].W;
                ...
                for (int j = 0; j < pointCount; ++j)
                {
                    ...
                    Fixed32 rnA = MathUtils.Cross(vcp.rA, vc.Normal);
                    Fixed32 rnB = MathUtils.Cross(vcp.rB, vc.Normal);

                    Fixed32 kNormal = mA + mB + iA * rnA * rnA + iB * rnB * rnB;

                    vcp.NormalMass = kNormal > Fixed32.Zero ? (Fixed32)1.0 / kNormal : Fixed32.Zero;

                    Vector2 tangent = MathUtils.Cross(vc.Normal, (Fixed32)1.0);

                    Fixed32 rtA = MathUtils.Cross(vcp.rA, tangent);
                    Fixed32 rtB = MathUtils.Cross(vcp.rB, tangent);

                    Fixed32 kTangent = mA + mB + iA * rtA * rtA + iB * rtB * rtB;

                    vcp.TangentMass = kTangent > Fixed32.Zero ? (Fixed32)1.0 / kTangent : Fixed32.Zero;

                    // Setup a velocity bias for restitution.
                    vcp.VelocityBias = Fixed32.Zero;
                    Fixed32 vRel = MathUtils.Dot(vc.Normal, vB + MathUtils.Cross(wB, vcp.rB) - vA - MathUtils.Cross(wA, vcp.rA));
                ...
                if (vc.PointCount == 2 && Settings.BlockSolve)
                {
                    ...
                    Fixed32 rn1A = MathUtils.Cross(vcp1.rA, vc.Normal);
                    Fixed32 rn1B = MathUtils.Cross(vcp1.rB, vc.Normal);
                    Fixed32 rn2A = MathUtils.Cross(vcp2.rA, vc.Normal);
                    Fixed32 rn2B = MathUtils.Cross(vcp2.rB, vc.Normal);

                    Fixed32 k11 = mA + mB + iA * rn1A * rn1A + iB * rn1B * rn1B;
                    Fixed32 k22 = mA + mB + iA * rn2A * rn2A + iB * rn2B * rn2B;
                    Fixed32 k12 = mA + mB + iA * rn1A * rn2A + iB * rn1B * rn2B;

                    // Ensure a reasonable condition number.
                    Fixed32 k_maxConditionNumber = (Fixed32)1000.0;
        ...
        public void WarmStart()
        {
            ...
            for (int i = 0; i < _count; ++i)
            {
                ...
                Vector2 tangent = MathUtils.Cross(normal, (Fixed32)1.0);
        ...
        public void SolveVelocityConstraints()
        {
            for (int i = 0; i < _count; ++i)
            {
                ...
                for (int j = 0; j < pointCount; ++j)
                {
                    ...
                    Fixed32 vt = Vector2.Dot(dv, tangent) - vc.TangentSpeed;
                    Fixed32 lambda = vcp.TangentMass * -vt;

                    // b2Clamp the accumulated force
                    Fixed32 maxFriction = friction * vcp.NormalImpulse;
                    Fixed32 newImpulse = MathUtils.Clamp(vcp.TangentImpulse + lambda, -maxFriction, maxFriction);
                ...
                if (pointCount == 1 || !Settings.BlockSolve)
                {
                    for (int j = 0; j < pointCount; ++j)
                    {
                        ...
                        Fixed32 vn = Vector2.Dot(dv, normal);
                        Fixed32 lambda = -vcp.NormalMass * (vn - vcp.VelocityBias);

                        // b2Clamp the accumulated impulse
                        Fixed32 newImpulse = Fixed32.Max(vcp.NormalImpulse + lambda, Fixed32.Zero);
                ...
                else
                {
                    ...
                    Debug.Assert(a.X >= Fixed32.Zero && a.Y >= Fixed32.Zero);
                    ...
                    Fixed32 vn1 = Vector2.Dot(dv1, normal);
                    Fixed32 vn2 = Vector2.Dot(dv2, normal);
                    ...
                    for (;;)
                    {
                        ...
                        if (x.X >= Fixed32.Zero && x.Y >= Fixed32.Zero)
                        {
                            ...
                            Debug.Assert(FMath.Abs(vn1 - cp1.VelocityBias) < k_errorTol);
                            Debug.Assert(FMath.Abs(vn2 - cp2.VelocityBias) < k_errorTol);
#endif
                            break;
                        }
                        ...
                        if (x.X >= Fixed32.Zero && vn2 >= Fixed32.Zero)
                        {
                            // Get the incremental impulse
                            Vector2 d = x - a;
                        ...
                        if (x.Y >= Fixed32.Zero && vn1 >= Fixed32.Zero)
                        {
                        ...
                        if (vn1 >= Fixed32.Zero && vn2 >= Fixed32.Zero)
                        {
                            // Resubstitute for the incremental impulse
                            Vector2 d = x - a;
        ...
        {
            Fixed32 minSeparation = Fixed32.Zero;
            ...
            for (int i = 0; i < _count; ++i)
            {
                ...
                Vector2 localCenterA = pc.LocalCenterA;
                Fixed32 mA = pc.InvMassA;
                Fixed32 iA = pc.InvIA;
                Vector2 localCenterB = pc.LocalCenterB;
                Fixed32 mB = pc.InvMassB;
                Fixed32 iB = pc.InvIB;
                ...
                for (int j = 0; j < pointCount; ++j)
                {
                    ...
                    PositionSolverManifold.Initialize(pc, ref xfA, ref xfB, j, out Vector2 normal, out Vector2 point, out Fixed32 separation);

                    Vector2 rA = point - cA;
                    Vector2 rB = point - cB;

                    // Track max constraint error.
                    minSeparation = Fixed32.Min(minSeparation, separation);

                    // Prevent large corrections and allow slop.
                    Fixed32 C = MathUtils.Clamp(Settings.Baumgarte * (separation + Settings.LinearSlop), -Settings.MaxLinearCorrection, Fixed32.Zero);

                    // Compute the effective mass.
                    Fixed32 rnA = MathUtils.Cross(rA, normal);
                    Fixed32 rnB = MathUtils.Cross(rB, normal);
                    Fixed32 K = mA + mB + iA * rnA * rnA + iB * rnB * rnB;

                    // Compute normal impulse
                    Fixed32 impulse = K > Fixed32.Zero ? -C / K : Fixed32.Zero;
            ...
            return minSeparation >= -(Fixed32)3.0 * Settings.LinearSlop;
        }
        ...
        public bool SolveTOIPositionConstraints(int toiIndexA, int toiIndexB)
        {
            ...
            for (int i = 0; i < _count; ++i)
            {
                ...
                Fixed32 mA = Fixed32.Zero;
                Fixed32 iA = Fixed32.Zero;
                ...
                Fixed32 mB = Fixed32.Zero;
                Fixed32 iB = Fixed32.Zero;
                ...
                for (int j = 0; j < pointCount; ++j)
                {
                    ...
                    // Prevent large corrections and allow slop.
                    Fixed32 C = MathUtils.Clamp(Settings.TOIBaumgarte * (separation + Settings.LinearSlop), -Settings.MaxLinearCorrection, Fixed32.Zero);
            ...
            // We can't expect minSpeparation >= -b2_linearSlop because we don't
            // push the separation above -b2_linearSlop.
            return minSeparation >= -(Fixed32)1.5 * Settings.LinearSlop;
        }
    }
}
