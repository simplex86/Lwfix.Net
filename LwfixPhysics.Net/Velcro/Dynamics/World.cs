...
using SimplexLab.LwfixPhysics.Velcro.Dynamics.Handlers;
...
namespace SimplexLab.LwfixPhysics.Velcro.Dynamics
{
    ...
    public class World
    {
        ...
        private Func<Fixture, Vector2, Vector2, Fixed32, Fixed32> _rayCastCallback;
        private Func<RayCastInput, int, Fixed32> _rayCastCallbackWrapper;
        ...
        public void Step(Fixed32 dt, int velocityIterations = 8, int positionIterations = 3)
        {
            ...
            if (dt > Fixed32.Zero)
                step.InvertedDeltaTime = (Fixed32)1.0 / dt;
            else
                step.InvertedDeltaTime = Fixed32.Zero;
            ...
            if (_stepComplete && step.DeltaTime > Fixed32.Zero)
            {
                Stopwatch timer = _timerPool.GetFromPool(true);
                Solve(ref step);
            ...
            // Handle TOI events.
            if (_continuousPhysicsEnabled && step.DeltaTime > Fixed32.Zero)
            {
                Stopwatch timer = _timerPool.GetFromPool(true);
                SolveTOI(ref step);
            ...
            if (step.DeltaTime > Fixed32.Zero)
        ...
        public void RayCast(Func<Fixture, Vector2, Vector2, Fixed32, Fixed32> callback, Vector2 point1, Vector2 point2)
        {
            RayCastInput input = new RayCastInput();
            input.MaxFraction = (Fixed32)1.0;
            input.Point1 = point1;
            input.Point2 = point2;
        ...
        public List<Fixture> RayCast(Vector2 point1, Vector2 point2)
        {
            ...
            Fixed32 RayCastCallback(Fixture f, Vector2 vector2, Vector2 vector3, Fixed32 f1)
            {
                affected.Add(f);
                return (Fixed32)1.0;
            }
        ...
        private Fixed32 RayCastCallbackWrapper(RayCastInput rayCastInput, int proxyId)
        {
            FixtureProxy proxy = _contactManager.BroadPhase.GetProxy(proxyId);
            ...
            if (hit)
            {
                Fixed32 fraction = output.Fraction;
                Vector2 point = ((Fixed32)1.0 - fraction) * rayCastInput.Point1 + fraction * rayCastInput.Point2;
        ...
        private void SolveTOI(ref TimeStep step)
        {
            ...
            if (_stepComplete)
            {
                for (int i = 0; i < _bodyList.Count; i++)
                {
                    ...
                    _bodyList[i]._sweep.Alpha0 = Fixed32.Zero;
                }
            ...
            for (; ; )
            {
                ...
                for (Contact c = _contactManager._contactList; c != null; c = c._next)
                {
                    ...
                    else
                    {
                        ...
                        Fixed32 alpha0 = bA._sweep.Alpha0;
                        ...
                        input.TMax = (Fixed32)1.0;

                        TimeOfImpact.CalculateTimeOfImpact(ref input, out TOIOutput output);

                        // Beta is the fraction of the remaining portion of the .
                        Fixed32 beta = output.T;
                        if (output.State == TOIOutputState.Touching)
                            alpha = Fixed32.Min(alpha0 + ((Fixed32)1.0 - alpha0) * beta, (Fixed32)1.0);
                        else
                            alpha = (Fixed32)1.0;
                ...
                if (minContact == null || (Fixed32)1.0 - (Fixed32)10.0 * MathConstants.Epsilon < minAlpha)
                {
                ...
                TimeStep subStep;
                subStep.DeltaTime = ((Fixed32)1.0 - minAlpha) * step.DeltaTime;
                subStep.InvertedDeltaTime = (Fixed32)1.0 / subStep.DeltaTime;
                subStep.DeltaTimeRatio = (Fixed32)1.0;
