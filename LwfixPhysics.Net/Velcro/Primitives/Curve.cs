...
namespace SimplexLab.LwfixPhysics.Velcro.Primitives
{
    ...
    internal class Curve
    {
        ...
        public Fixed32 Evaluate(Fixed32 position)
        {
            if (Keys.Count == 0)
                return Fixed32.Zero;
            ...
            if (position < first.Position)
            {
                switch (PreLoop)
                {
                    ...
                    case CurveLoopType.Cycle:
                        ...
                        Fixed32 virtualPos = position - cycle * (last.Position - first.Position);
                    ...
                    case CurveLoopType.Oscillate:
                        ...
                        // start-> end / end -> start
                        cycle = GetNumberOfCycle(position);
                        if (0 == cycle % Fixed32.Two) //if pair
                            virtualPos = position - cycle * (last.Position - first.Position);
                        else
        ...
        public void ComputeTangent(int keyIndex, CurveTangent tangentInType, CurveTangent tangentOutType)
        {
            ...
            switch (tangentInType)
            {
                ...
                case CurveTangent.Smooth:
                    Fixed32 pn = p1 - p0;
                    if (FMath.Abs(pn) < Fixed32.Epsilon)
                        key.TangentIn = 0;
                    else
        ...
        private int GetNumberOfCycle(Fixed32 position)
        {
            Fixed32 cycle = (position - Keys[0].Position) / (Keys[Keys.Count - 1].Position - Keys[0].Position);
            if (cycle < Fixed32.Zero)
                cycle = cycle - Fixed32.One;
            return (int)cycle;
        }

        private Fixed32 GetCurvePosition(Fixed32 position)
        {
            ...
            for (int i = 1; i < Keys.Count; ++i)
            {
                ...
                if (next.Position >= position)
                {
                    ...
                    {
                        if (position >= Fixed32.One)
                            return next.Value;
                        return prev.Value;
                    }
                    Fixed32 t = (position - prev.Position) / (next.Position - prev.Position); //to have t in [0,1]
                    Fixed32 ts = t * t;
                    Fixed32 tss = ts * t;
                    ...
                    return (2 * tss - 3 * ts + Fixed32.One) * prev.Value + (tss - 2 * ts + t) * prev.TangentOut + (3 * ts - 2 * tss) * next.Value + (tss - ts) * next.TangentIn;
                }
                prev = next;
            }
            return Fixed32.Zero;
