...
namespace SimplexLab.LwfixPhysics.Velcro.Shared
{
    ...
    public struct AABB
    {
        ...
        public AABB(Vector2 center, Fixed32 width, Fixed32 height)
            : this(center - new Vector2(width / 2, height / 2), center + new Vector2(width / 2, height / 2)) { }

        public AABB(ref Vector2 min, ref Vector2 max)
        {
            LowerBound = new Vector2(Fixed32.Min(min.X, max.X), Fixed32.Min(min.Y, max.Y));
            UpperBound = new Vector2(Fixed32.Max(min.X, max.X), Fixed32.Max(min.Y, max.Y));
        }

        public Fixed32 Width => UpperBound.X - LowerBound.X;

        public Fixed32 Height => UpperBound.Y - LowerBound.Y;

        /// <summary>Get the center of the AABB.</summary>
        public Vector2 Center => Fixed32.Half * (LowerBound + UpperBound);

        /// <summary>Get the extents of the AABB (half-widths).</summary>
        public Vector2 Extents => Fixed32.Half * (UpperBound - LowerBound);

        /// <summary>Get the perimeter length</summary>
        public Fixed32 Perimeter
        {
            get
            {
                Fixed32 wx = UpperBound.X - LowerBound.X;
                Fixed32 wy = UpperBound.Y - LowerBound.Y;
                return Fixed32.Two * (wx + wy);
            }
        }
        ...
        public bool IsValid()
        {
            ...
            bool valid = d.X >= Fixed32.Zero && d.Y >= Fixed32.Zero;
        ...
        public bool Contains(ref Vector2 point)
        {
            ...
            return (point.X > (LowerBound.X + Fixed32.Epsilon) && point.X < (UpperBound.X - Fixed32.Epsilon) &&
                    (point.Y > (LowerBound.Y + Fixed32.Epsilon) && point.Y < (UpperBound.Y - Fixed32.Epsilon)));
        }
        ...
        public bool RayCast(ref RayCastInput input, out RayCastOutput output, bool doInteriorCheck = true)
        {
            ...
            Fixed32 tmin = -MathConstants.MaxFloat;
            Fixed32 tmax = MathConstants.MaxFloat;
            ...
            for (int i = 0; i < 2; ++i)
            {
                Fixed32 absD_i = i == 0 ? absD.X : absD.Y;
                Fixed32 lowerBound_i = i == 0 ? LowerBound.X : LowerBound.Y;
                Fixed32 upperBound_i = i == 0 ? UpperBound.X : UpperBound.Y;
                Fixed32 p_i = i == 0 ? p.X : p.Y;
                ...
                {
                    Fixed32 d_i = i == 0 ? d.X : d.Y;

                    Fixed32 inv_d = Fixed32.One / d_i;
                    Fixed32 t1 = (lowerBound_i - p_i) * inv_d;
                    Fixed32 t2 = (upperBound_i - p_i) * inv_d;

                    // Sign of the normal vector.
                    Fixed32 s = Fixed32.NegativeOne;
            ...
            if (doInteriorCheck && (tmin < Fixed32.Zero || input.MaxFraction < tmin))
                return false;
