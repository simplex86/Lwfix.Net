...
namespace SimplexLab.LwfixPhysics.Velcro.Shared
{
    ...
    public class Vertices : List<Vector2>
    {
        ...
        /// <returns></returns>
        public Fixed32 GetArea()
        {
            Fixed32 area = GetSignedArea();
        ...
        public Vector2 GetCentroid()
        {
            //The simplest polygon which can exist in the Euclidean plane has 3 sides.
            if (Count < 3)
                return new Vector2(Fixed32.NaN, Fixed32.NaN);

            // Same algorithm is used by Box2D
            Vector2 c = Vector2.Zero;
            Fixed32 area = Fixed32.Zero;
            Fixed32 inv3 = Fixed32.One / (Fixed32)3.0;
            ...
            for (int i = 0; i < Count; ++i)
            {
                ...
                Fixed32 triangleArea = Fixed32.Half * (current.X * next.Y - current.Y * next.X);
                area += triangleArea;
        ...
        {
            AABB aabb;
            Vector2 lowerBound = new Vector2(Fixed32.MaxValue, Fixed32.MaxValue);
            Vector2 upperBound = new Vector2(Fixed32.MinValue, Fixed32.MinValue);
        ...
        public bool IsConvex()
        {
            ...
            for (int i = 0; i < Count; ++i)
            {
                ...
                for (int j = 0; j < Count; ++j)
                {
                    ...
                    Fixed32 s = edge.X * r.Y - edge.Y * r.X;
        ...
        public bool IsCounterClockWise()
        {
            ...
            return GetSignedArea() > Fixed32.Zero;
        }
        ...
        internal PolygonError CheckPolygon()
        {
            ...
            for (int i = 0; i < Count; ++i)
            {
                int next = i + 1 < Count ? i + 1 : 0;
                Vector2 edge = this[next] - this[i];
                if (edge.LengthSquared() <= Fixed32.Epsilon * Fixed32.Epsilon)
        ...
        public void ProjectToAxis(ref Vector2 axis, out Fixed32 min, out Fixed32 max)
        {
            // To project a point on an axis use the dot product
            Fixed32 dotProduct = Vector2.Dot(axis, this[0]);
            min = dotProduct;
            max = dotProduct;
        ...
        public int PointInPolygon(ref Vector2 point)
        {
            ...
            for (int i = 0; i < Count; i++)
            {
                ...
                Vector2 edge = p2 - p1;
                Fixed32 area = MathUtils.Area(ref p1, ref p2, ref point);
                if (area == Fixed32.Zero && Vector2.Dot(point - p1, edge) >= Fixed32.Zero && Vector2.Dot(point - p2, edge) <= Fixed32.Zero)
                    return 0;

                // Test edge for intersection with ray from point
                if (p1.Y <= point.Y)
                {
                    if (p2.Y > point.Y && area > Fixed32.Zero)
                        ++wn;
                }
                else
                {
        ...
        public bool PointInPolygonAngle(ref Vector2 point)
        {
            ...
            if (FMath.Abs(angle) < MathConstants.Pi)
                return false;
