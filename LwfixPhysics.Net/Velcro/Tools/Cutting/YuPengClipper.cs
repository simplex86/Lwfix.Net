...
using SimplexLab.LwfixPhysics.Velcro.Tools.Cutting.Simple;
using SimplexLab.LwfixPhysics.Velcro.Tools.PolygonManipulation;
using SimplexLab.LwfixPhysics.Velcro.Utilities;

namespace SimplexLab.LwfixPhysics.Velcro.Tools.Cutting
{
    //Clipper contributed by Helge Backhaus

    public static class YuPengClipper
    {
        private static readonly Fixed32 ClipperEpsilonSquared = (Fixed32)1.192092896e-07;
        ...
        private static List<Vertices> Execute(Vertices subject, Vertices clip, PolyClipType clipType, out PolyClipError error)
        {
            ...
            CalculateSimplicalChain(slicedSubject, out List<Fixed32> subjectCoeff, out List<Edge> subjectSimplices);
            CalculateSimplicalChain(slicedClip, out List<Fixed32> clipCoeff, out List<Edge> clipSimplices);
            ...
            for (int i = 0; i < polygon1.Count; i++)
            {
                ...
                for (int j = 0; j < polygon2.Count; j++)
                {
                    ...
                    if (LineUtils.LineIntersect(a, b, c, d, out Vector2 intersectionPoint))
                    {
                        ...
                        Fixed32 alpha = GetAlpha(a, b, intersectionPoint);

                        // Insert intersection point into first polygon
                        if (alpha > Fixed32.Zero && alpha < Fixed32.One)
                        {
        ...
        private static void CalculateSimplicalChain(Vertices poly, out List<Fixed32> coeff, out List<Edge> simplicies)
        {
            simplicies = new List<Edge>();
            coeff = new List<Fixed32>();
        ...
        /// </summary>
        /// <remarks>Used by method <c>Execute()</c>.</remarks>
        private static void CalculateResultChain(List<Fixed32> poly1Coeff, List<Edge> poly1Simplicies, List<Fixed32> poly2Coeff, List<Edge> poly2Simplicies, PolyClipType clipType, out List<Edge> resultSimplices)
        {
            resultSimplices = new List<Edge>();

            for (int i = 0; i < poly1Simplicies.Count; ++i)
            {
                Fixed32 edgeCharacter = Fixed32.Zero;
                if (poly2Simplicies.Contains(poly1Simplicies[i]))
                    edgeCharacter = Fixed32.One;
                ...
                {
                    if (edgeCharacter == Fixed32.One)
                        resultSimplices.Add(poly1Simplicies[i]);
                }
                else
                {
                    if (edgeCharacter == Fixed32.Zero)
                        resultSimplices.Add(poly1Simplicies[i]);
                }
            }
        ...
        private static Fixed32 CalculateBeta(Vector2 point, Edge e, Fixed32 coefficient)
        {
            Fixed32 result = Fixed32.Zero;
            ...
            if (PointOnLineSegment(Vector2.Zero, e.EdgeStart, point) ||
                ...
                result = Fixed32.Half * coefficient;
            return result;
        }

        /// <summary>Needed for sorting multiple intersections points on the same edge.</summary>
        /// <remarks>Used by method <c>CalculateIntersections()</c>.</remarks>
        private static Fixed32 GetAlpha(Vector2 start, Vector2 end, Vector2 point)
        {
        ...
        private static Fixed32 CalculateSimplexCoefficient(Vector2 a, Vector2 b, Vector2 c)
        {
            Fixed32 isLeft = MathUtils.Area(ref a, ref b, ref c);
            if (isLeft < Fixed32.Zero)
                return Fixed32.NegativeOne;
        ...
        private static bool PointOnLineSegment(Vector2 start, Vector2 end, Vector2 point)
        {
            ...
            return MathUtils.Area(ref start, ref end, ref point) == Fixed32.Zero &&
                   Vector2.Dot(point - start, segment) >= Fixed32.Zero &&
                   Vector2.Dot(point - end, segment) <= Fixed32.Zero;
        }
        ...
        private sealed class Edge
        {
            ...
            {
                return (EdgeStart + EdgeEnd) / Fixed32.Two;
            }
