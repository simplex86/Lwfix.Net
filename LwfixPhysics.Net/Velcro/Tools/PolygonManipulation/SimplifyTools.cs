...
namespace SimplexLab.LwfixPhysics.Velcro.Tools.PolygonManipulation
{
    ...
    public static class SimplifyTools
    {
        ...
        public static Vertices CollinearSimplify(Vertices vertices, Fixed32 collinearityTolerance = default)
        {
            if (vertices.Count <= 3)
                return vertices;
        ...
        public static Vertices DouglasPeuckerSimplify(Vertices vertices, Fixed32 distanceTolerance)
        {
            if (vertices.Count <= 3)
                return vertices;
        ...
        private static void SimplifySection(Vertices vertices, int i, int j, bool[] usePoint, Fixed32 distanceTolerance)
        {
            if (i + 1 == j)
                return;
            ...
            Fixed32 maxDistance = Fixed32.NegativeOne;
            int maxIndex = i;
            for (int k = i + 1; k < j; k++)
            {
                Vector2 point = vertices[k];

                Fixed32 distance = LineUtils.DistanceBetweenPointAndLineSegment(ref point, ref a, ref b);
        ...
        public static Vertices MergeParallelEdges(Vertices vertices, Fixed32 tolerance)
        {
            ...
            for (int i = 0; i < vertices.Count; ++i)
            {
                ...
                Fixed32 dx0 = vertices[middle].X - vertices[lower].X;
                Fixed32 dy0 = vertices[middle].Y - vertices[lower].Y;
                Fixed32 dx1 = vertices[upper].X - vertices[middle].X;
                Fixed32 dy1 = vertices[upper].Y - vertices[middle].Y;
                Fixed32 norm0 = Fixed32.Sqrt(dx0 * dx0 + dy0 * dy0);
                Fixed32 norm1 = Fixed32.Sqrt(dx1 * dx1 + dy1 * dy1);

                if (!(norm0 > Fixed32.Zero && norm1 > Fixed32.Zero) && newNVertices > 3)
                {
                ...
                dx1 /= norm1;
                dy1 /= norm1;
                Fixed32 cross = dx0 * dy1 - dx1 * dy0;
                Fixed32 dot = dx0 * dx1 + dy0 * dy1;

                if (FMath.Abs(cross) < tolerance && dot > 0 && newNVertices > 3)
                {
                    mergeMe[i] = true;
                    --newNVertices;
                }
        ...
        public static Vertices ReduceByDistance(Vertices vertices, Fixed32 distance)
        {
            if (vertices.Count <= 3)
                return vertices;

            Fixed32 distance2 = distance * distance;
        ...
        /// <returns></returns>
        public static Vertices ReduceByArea(Vertices vertices, Fixed32 areaTolerance)
        {
            //From physics2d.net
            ...
            for (int i = 0; i < vertices.Count; ++i, v2 = v3)
            {
                ...
                MathUtils.Cross(ref v1, ref v2, out Fixed32 old1);

                MathUtils.Cross(ref v2, ref v3, out Fixed32 old2);

                MathUtils.Cross(ref v1, ref v3, out Fixed32 new1);

                if (FMath.Abs(new1 - (old1 + old2)) > areaTolerance)
                {
                    simplified.Add(v2);
                    v1 = v2;
                }
