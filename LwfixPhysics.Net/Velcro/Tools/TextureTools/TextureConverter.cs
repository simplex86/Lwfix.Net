...
namespace SimplexLab.LwfixPhysics.Velcro.Tools.TextureTools
{
    ...
    public sealed class TextureConverter
    {
        ...
        private Fixed32 _hullTolerance;
        ...
        private void Initialize(uint[] data, int? width, byte? alphaTolerance, Fixed32? hullTolerance, bool? holeDetection, bool? multipartDetection, bool? pixelOffsetOptimization, Matrix? transform)
        {
        ...
        /// <returns></returns>
        public static List<Vertices> DetectVertices(uint[] data, int width, Fixed32 hullTolerance, byte alphaTolerance, bool multiPartDetection, bool holeDetection)
        {
            TextureConverter tc =
        ...
        public List<Vertices> DetectVertices()
        {
            ...
            do
            {
                ...
                else if (polygonEntrance.HasValue)
                {
                    ...
                    polygon = new Vertices(CreateSimplePolygon(polygonEntrance.Value, new Vector2(polygonEntrance.Value.X - Fixed32.One, polygonEntrance.Value.Y)));
                }
                else
                    break;
        ...
        private bool InPolygon(Vertices polygon, Vector2 point)
        {
            ...
            if (!inPolygon)
            {
                List<Fixed32> xCoords = SearchCrossingEdgesHoles(polygon, (int)point.Y);
        ...
        {
            Fixed32 topMostValue = Fixed32.MaxValue;
        ...
        private Fixed32 GetTopMostCoord(Vertices vertices)
        {
            Fixed32 returnValue = Fixed32.MaxValue;
        ...
        private Fixed32 GetBottomMostCoord(Vertices vertices)
        {
            Fixed32 returnValue = Fixed32.MinValue;
        ...
        private List<Fixed32> SearchCrossingEdgesHoles(Vertices polygon, int y)
        {
            if (polygon == null)
                throw new ArgumentNullException(nameof(polygon), "'polygon' can't be null.");
            ...
            List<Fixed32> result = SearchCrossingEdges(polygon, y);
        ...
        private List<Fixed32> SearchCrossingEdges(Vertices polygon, int y)
        {
            // sick-o-note:
            ...
            List<Fixed32> edges = new List<Fixed32>();
        ...
        private bool SplitPolygonEdge(Vertices polygon, Vector2 coordInsideThePolygon, out int vertex1Index, out int vertex2Index)
        {
            ...
            Fixed32 shortestDistance = Fixed32.MaxValue;

            bool edgeCoordFound = false;
            Vector2 foundEdgeCoord = Vector2.Zero;

            List<Fixed32> xCoords = SearchCrossingEdges(polygon, (int)coordInsideThePolygon.Y);
            ...
            if (xCoords != null && xCoords.Count > 1 && xCoords.Count % 2 == 0)
            {
                ...
                if (edgeCoordFound)
                {
                    shortestDistance = Fixed32.MaxValue;
        ...
        private Vertices CreateSimplePolygon(Vector2 entrance, Vector2 last)
        {
            ...
            if (entrance == Vector2.Zero || !InBounds(ref entrance))
            {
                ...
                if (entranceFound)
                {
                    current = new Vector2(entrance.X - Fixed32.One, entrance.Y);
                }
            }
            else
            {
        ...
        private bool SearchNextHullEntrance(List<Vertices> detectedPolygons, Vector2 start, out Vector2? entrance)
        {
            ...
            for (int i = (int)start.X + (int)start.Y * _width; i <= _dataLength; i++)
            {
                if (IsSolid(ref i))
                {
                    ...
                    {
                        x = i % _width;
                        entrance = new Vector2(x, (i - x) / (Fixed32)_width);
        ...
        public Fixed32 HullTolerance
        {
            get { return _hullTolerance; }
            set
            {
                if (value > (Fixed32)4.0)
                {
                    _hullTolerance = (Fixed32)4.0;
                }
                else if (value < (Fixed32)0.9)
                {
                    _hullTolerance = (Fixed32)0.9;
                }
                else
                {
        ...
        {
            return (coord.X >= Fixed32.Zero && coord.X < _width && coord.Y >= Fixed32.Zero && coord.Y < _height);
        }
    }
}
