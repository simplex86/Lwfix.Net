...
namespace SimplexLab.LwfixPhysics.Velcro.Tools.Triangulation.Earclip
{
    ...
    internal static class EarclipDecomposer
    {
        ...
        public static List<Vertices> ConvexPartition(Vertices vertices, Fixed32 tolerance = default)
        {
        ...
        private static List<Vertices> TriangulatePolygon(Vertices vertices, Fixed32 tolerance)
        {
            ...
            int bufferSize = 0;
            Fixed32[] xrem = new Fixed32[vertices.Count];
            Fixed32[] yrem = new Fixed32[vertices.Count];
            ...
            while (vNum > 3)
            {
                // Find an ear
                int earIndex = -1;
                Fixed32 earMaxMinCross = (Fixed32)(-10.0);
                for (int i = 0; i < vNum; ++i)
                {
                    if (IsEar(i, xrem, yrem, vNum))
                    {
                        ...
                        d2.Normalize();
                        d3.Normalize();
                        MathUtils.Cross(ref d1, ref d2, out Fixed32 cross12);
                        cross12 = FMath.Abs(cross12);

                        MathUtils.Cross(ref d2, ref d3, out Fixed32 cross23);
                        cross23 = FMath.Abs(cross23);

                        MathUtils.Cross(ref d3, ref d1, out Fixed32 cross31);
                        cross31 = FMath.Abs(cross31);

                        //Find the maximum minimum angle
                        Fixed32 minCross = FMath.Min(cross12, FMath.Min(cross23, cross31));
                ...
                --vNum;
                Fixed32[] newx = new Fixed32[vNum];
                Fixed32[] newy = new Fixed32[vNum];
                int currDest = 0;
                for (int i = 0; i < vNum; ++i)
                {
        ...
        private static bool ResolvePinchPoint(Vertices pin, out Vertices poutA, out Vertices poutB, Fixed32 tolerance)
        {
            poutA = new Vertices();
            poutB = new Vertices();
            ...
            for (int i = 0; i < pin.Count; ++i)
            {
                for (int j = i + 1; j < pin.Count; ++j)
                {
                    ...
                    if (FMath.Abs(pin[i].X - pin[j].X) < tolerance && FMath.Abs(pin[i].Y - pin[j].Y) < tolerance && j != i + 1)
                    {
                        pinchIndexA = i;
                        pinchIndexB = j;
                        hasPinchPoint = true;
        ...
        private static bool IsEar(int i, Fixed32[] xv, Fixed32[] yv, int xvLength)
        {
            Fixed32 dx0, dy0, dx1, dy1;
