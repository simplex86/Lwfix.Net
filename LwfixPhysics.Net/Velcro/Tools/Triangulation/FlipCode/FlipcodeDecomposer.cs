...
namespace SimplexLab.LwfixPhysics.Velcro.Tools.Triangulation.FlipCode
{
    /// <summary>
    ...
    internal static class FlipcodeDecomposer
    {
        ...
        {
            //A cross bp
            Fixed32 abp = (c.X - b.X) * (p.Y - b.Y) - (c.Y - b.Y) * (p.X - b.X);

            //A cross ap
            Fixed32 aap = (b.X - a.X) * (p.Y - a.Y) - (b.Y - a.Y) * (p.X - a.X);

            //b cross cp
            Fixed32 bcp = (a.X - c.X) * (p.Y - c.Y) - (a.Y - c.Y) * (p.X - c.X);

            return abp >= Fixed32.Zero && bcp >= Fixed32.Zero && aap >= Fixed32.Zero;
        }
