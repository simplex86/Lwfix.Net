...
namespace SimplexLab.LwfixPhysics.Velcro.Tools.Triangulation.Seidel
{
    ...
    {
        // Almost Pi!
        private static readonly Fixed32 PiSlop = (Fixed32)3.1;
        ...
        public void Process()
        {
            ...
            while (p.Neq(_tail))
            {
                ...
                if (a >= PiSlop || a <= -PiSlop || a == Fixed32.Zero)
                    Remove(p);
                else if (IsConvex(p))
                    _convexPoints.Add(p);
                p = p.Next;
        ...
        private Fixed32 Angle(Point p)
        {
            Point a = p.Next - p;
            Point b = p.Prev - p;
            return Fixed32.Atan2(a.Cross(b), a.Dot(b));
        }
        ...
        private bool AngleSign()
        {
            ...
            Point b = _tail - _head;
            return Fixed32.Atan2(a.Cross(b), a.Dot(b)) >= 0;
        }
