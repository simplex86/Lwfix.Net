...
namespace SimplexLab.LwfixPhysics.Velcro.Utilities
{
    ...
    public static class LineUtils
    {
        public static Fixed32 DistanceBetweenPointAndLineSegment(ref Vector2 point, ref Vector2 start, ref Vector2 end)
        {
            ...
            Fixed32 c1 = Vector2.Dot(w, v);
            if (c1 <= 0)
                return Vector2.Distance(point, start);

            Fixed32 c2 = Vector2.Dot(v, v);
            if (c2 <= c1)
        ...
        public static bool LineIntersect2(ref Vector2 a0, ref Vector2 a1, ref Vector2 b0, ref Vector2 b1, out Vector2 intersectionPoint)
        {
            ...
            //AABB early exit
            if (Fixed32.Max(x1, x2) < Fixed32.Min(x3, x4) || Fixed32.Max(x3, x4) < Fixed32.Min(x1, x2))
                return false;

            if (Fixed32.Max(y1, y2) < Fixed32.Min(y3, y4) || Fixed32.Max(y3, y4) < Fixed32.Min(y1, y2))
                return false;

            Fixed32 ua = (x4 - x3) * (y1 - y3) - (y4 - y3) * (x1 - x3);
            Fixed32 ub = (x2 - x1) * (y1 - y3) - (y2 - y1) * (x1 - x3);
            Fixed32 denom = (y4 - y3) * (x2 - x1) - (x4 - x3) * (y2 - y1);
            if (FMath.Abs(denom) < MathConstants.Epsilon)
            {
        ...
        public static Vector2 LineIntersect(Vector2 p1, Vector2 p2, Vector2 q1, Vector2 q2)
        {
            ...
            Fixed32 a1 = p2.Y - p1.Y;
            Fixed32 b1 = p1.X - p2.X;
            Fixed32 c1 = a1 * p1.X + b1 * p1.Y;
            Fixed32 a2 = q2.Y - q1.Y;
            Fixed32 b2 = q1.X - q2.X;
            Fixed32 c2 = a2 * q1.X + b2 * q1.Y;
            Fixed32 det = a1 * b2 - a2 * b1;
        ...
        public static bool LineIntersect(ref Vector2 point1, ref Vector2 point2, ref Vector2 point3, ref Vector2 point4, bool firstIsSegment, bool secondIsSegment, out Vector2 point)
        {
            ...
            Fixed32 a = point4.Y - point3.Y;
            Fixed32 b = point2.X - point1.X;
            Fixed32 c = point4.X - point3.X;
            Fixed32 d = point2.Y - point1.Y;

            // denominator to solution of linear system
            Fixed32 denom = (a * b) - (c * d);

            // if denominator is 0, then lines are parallel
            if (!(denom >= -MathConstants.Epsilon && denom <= MathConstants.Epsilon))
            {
                Fixed32 e = point1.Y - point3.Y;
                Fixed32 f = point1.X - point3.X;
                Fixed32 oneOverDenom = (Fixed32)1.0 / denom;
                ...
                if (!firstIsSegment || ua >= Fixed32.Zero && ua <= (Fixed32)1.0)
                {
                    ...
                    // segment 1 as well.
                    if (!secondIsSegment || ub >= Fixed32.Zero && ub <= (Fixed32)1.0)
                    {
                        // check if they are coincident (no collision in this case)
                        if (ua != Fixed32.Zero || ub != Fixed32.Zero)
                        {
        ...
        /// adjacent points, so for example edges one and two are between the first and second vertices and between the second and
        /// third vertices. The last edge is between vertex vertices.Count - 1 and verts0. (ie, vertices from a Geometry or AABB)
        /// </summary>
